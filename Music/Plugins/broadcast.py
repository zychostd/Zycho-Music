...
from Music.MusicUtilities.database.broadcast import db, dcmdb
from Music.config import BROADCAST_AS_COPY, OWNER_ID, LOG_CHANNEL_ID

@app.on_message()
async def _(bot: Client, cmd: Message):
    await handle_user_status(bot, cmd)

@app.on_message(filters.command("share") & filters.user(OWNER_ID) & filters.reply)
    ...
    if ban_status["is_banned"]:
        if (
            ...
            await cmd.reply_text(
                f"sorry, you're banned, ask to @UnrealZlda if you think this was an mistake.",
                quote=True,
            )
            return
